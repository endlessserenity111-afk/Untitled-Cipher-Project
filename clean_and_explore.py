...
skeleton, not a guaranteed drop-in):
  ...
  - Raw files are CSV or can be unzipped from dataset.zip into CSV.
...
DATASET_ZIP    = 'dataset.zip'   # if your raw CSVs are inside a zip instead
...
def _maybe_extract_zip():
    """If the raw CSVs aren't present but dataset.zip is, unzip it first."""
