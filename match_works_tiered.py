...
====================================
Matches completed MPLADS works to their recommended counterparts using a
...
def print_summary(df_m, df_t2, df_u, total):
    ...
    if n > 0:
        ...
        for idx, (_, row) in enumerate(df_m.sample(min(10, n), random_state=77).iterrows(), 1):
            ...
            print(f"  COMP  (ID {row['Completed Work ID']}): {row['Completed Work Description']}")
            print(f"  REC   (ID {row['Matched Recommended Work ID']}): {row['Matched Work Description']}")
            print("-"*85)
